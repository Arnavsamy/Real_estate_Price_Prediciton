...
def get_scaled_values(input_dict):
    ...
    for key, value in input_dict.items():
        max_val = float(X[key].max())  # Ensure it's a float
        min_val = float(X[key].min())  # Ensure it's a float
...
def main() :  
    ...
    with col1:
        ...
        st.write("`This Artificial Intelligence can Assist Price Predicitons `")
